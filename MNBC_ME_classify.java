	...
	private static void readBaseOutputFile(File outputFile) {
		...
		try {
			...
			while((line = reader.readLine()) != null) {
				...
				if(fields.length == 11 || line.contains("unclassified")) {
	...
	private static class Consumer implements Runnable {
		...
		private void processReadMinimizers(MutableIntSet readMinimizers) throws Exception {
			...
			if(votingGenomes.size() == 1) {
				...
				for(String taxId : completeGenomeId2TaxIds.get(predictedGenomeId)) {
					outcome += "\t" + taxId;
				}
				...
				while(it.hasNext()) {
					...
					String[] taxIds = completeGenomeId2TaxIds.get(genomeId);
					if(taxIds[0].equals("plasmid")) {
						plasmid.add(genomeId);						
					} else if(taxIds[0].equals("virus")){
						virus.add(genomeId);						
					} else {
						chro.add(genomeId);
					}
